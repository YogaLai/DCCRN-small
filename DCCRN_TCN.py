...
if __name__ == '__main__':
    ...
    inputs = torch.randn([10,16000*4]).clamp_(-1,1).cuda()
    labels = torch.randn([10,16000*4]).clamp_(-1,1)
    
    
    # DCCRN-E
    net = DCCRN(rnn_units=256,masking_mode='E').cuda()
    ...
    # outputs = net(inputs)
    # loss = net.loss(outputs, labels, loss_mode='MSE')
    # print(loss)

    # Test CRM
    stft = STFT(512, 100, 400, 'complex')
    noisy = stft.wav2spec(inputs)
    clean = stft.wav2spec(labels)
    crm = get_crm(noisy, clean)
